#+title: Emacs Canvas Patch

#+html: <img src="https://github.com/minad/emacs-canvas-patch/blob/main/screenshot.png?raw=true"/>

* Installation

The Canvas patch is part of the Emacs 32 source tree. The instructions here
apply only to Emacs 31.

1. Apply canvas-31.patch to the Emacs source depending on
   your Emacs version.
2. Recompile Emacs.
2. emacs -Q -l canvas-demo.el

* Canvas links

- [[https://debbugs.gnu.org/cgi/bugreport.cgi?bug=80281][bug#80281]]
- [[https://github.com/minad/doom-on-emacs][doom-on-emacs]]
- [[https://github.com/minad/emacs-canvas-patch][emacs-canvas-patch]]
- [[https://github.com/minad/emacs-shader-demo][emacs-shader-demo]]
- [[https://tusharhero.codeberg.page/emacs-pale-canvas-and-stuff-demos.html][Pale and other demos]]
